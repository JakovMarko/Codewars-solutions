...
class Song{
    ...
    howMany(arr){
      let newL = 0;
      let previous = this.listeners.length
      let songListeners = arr.map(item => {
        item.toLowerCase()
     if(this.listeners.indexOf(item.toLowerCase()) == -1){
      this.listeners.push(item.toLowerCase());
       newL++;
       }
      
    });
      console.log(this.listeners, newL, arr.length)
      return newL;
    }
  }
